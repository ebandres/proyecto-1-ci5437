...
void set_pdb(){
	FILE *f1 = fopen("absTopSpin_1.pdb","r");
	FILE *f2 = fopen("absTopSpin_2.pdb","r");

	pdb1 = read_state_map(f1);
	pdb2 = read_state_map(f2);

	abs1 = read_abstraction_from_file("absTopSpin_1.abst");
   abs2 = read_abstraction_from_file("absTopSpin_2.abst");
